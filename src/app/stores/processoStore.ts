...
export const useProcessoStore = defineStore('processo', () => {
  ...
  async function criarProcesso(dadosProcesso: Omit<Processo, 'id' | 'dataCriacao' | 'dataAtualizacao' | 'movimentacoes' | 'documentos'>) {
    try {
      salvando.value = true;
      erro.value = null;
  ...
  function filtrarPorTipo(tipos: TipoProcesso[]): Processo[] {
    ...
    return processos.value.filter(p => tipos.includes(p.tipo));
  }
